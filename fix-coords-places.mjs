...
console.log(`Processing ${stores.length} stores with place_id...`);
...
for (const store of stores) {
  ...
  if (moved) {
    ...
    if (error) {
      console.log(`❌ UPDATE ERROR: ${store.name}: ${error.message}`);
      errors++;
    } else {
      console.log(`🔴 FIXED: ${store.name}`);
      console.log(`   ${store.lat},${store.lng} → ${geo.lat},${geo.lng}`);
      updated++;
    }
  } else {
    console.log(`✅ OK: ${store.name} (${geo.lat},${geo.lng})`);
    skipped++;
  }
