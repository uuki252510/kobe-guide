    ...
    <div className="min-h-screen bg-harbor-950 max-w-lg mx-auto pb-24">
      <div className="sticky top-0 z-10 bg-harbor-950/95 backdrop-blur-sm border-b border-harbor-800 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-harbor-500 text-xs">フォロー中 {following.length}人</p>
        </div>
      </div>

      <div className="p-4 space-y-2">
        {following.length === 0 ? (
          <p className="text-center text-harbor-500 text-sm py-12">まだフォローしているユーザーがいません</p>
        ) : (
          following.map(({ id, profile: p }) =>
